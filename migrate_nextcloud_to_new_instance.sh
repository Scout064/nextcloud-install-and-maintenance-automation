...
#Ask if remote root login is allowed on the old Instance
read -p "Is remote root login via SSH allowed on the old Instance? [Y/N] " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Nn]$ ]]
then
	echo "This script only works if remote SSH login for root is allowed!"
	echo "exiting ... Please do the migration manually or enable remote login for root!"
	sleep 10
	exit
else
...
rootusr='root'
# Ask the user for Data of old Instance
echo 
echo "Please put in the IP-Address of the old Instance"
read ipold
echo # (optional) move to a new line
echo "Please put in the root Password of the old Instance"
read oldrootpw
echo # (optional) move to a new line
echo "Please put in the db name of the old Instance"
read dbold
echo # (optional) move to a new line
echo "Please put in the db root passwort of the old Instance"
read dbrootoldpw
echo # (optional) move to a new line
echo "Please put in the nextcloud data path of the old Instance"
read ncpathold
echo # (optional) move to a new line
...
#inform user that we will move on with the migration
echo 
echo "You answered 'Y', assuming the Wizard has been completed!"
sleep 5
echo "Begin preparing the Instance for migration!"
sleep 2
#put nc to maintenance mode
cd $ncpathnew
...
#Ask for confirmation to move on
echo "We are done collecting data. Now we can start the migration."
echo "Please make sure the old Instance is set to Maintenance mode."
echo "while having cd'ed into the nextcloud path, do sudo -u www-data php occ maintenance:mode --on"
read -p "Do you want to continue? [Y/N] " -n 1 -r
echo    # (optional) move to a new line
if [[ $REPLY =~ ^[Yy]$ ]]
then
    #start migration with backup of old db
	sshpass -p $oldrootpw ssh $rootusr@$ipold bash -c "'mysqldump --single-transaction -u $rootusr -p$dbrootoldpw $dbold > /root/nextcloud-sqlbkp.bak'"
	#pull db backup and import to new instance
	sshpass -p $oldrootpw rsync $rootusr@$ipold:/root/nextcloud-sqlbkp.bak /
	mysql -u $rootusr -p$dbrootnewpw -e "DROP DATABASE nextcloud"
	mysql -u $rootusr -p$dbrootnewpw -e "CREATE DATABASE nextcloud"
	mysql -u $rootusr -p$dbrootnewpw nextcloud < nextcloud-sqlbkp.bak
	#pull Nextclouddata from old Instance
	sshpass -p $oldrootpw rsync -a -t $rootusr@$ipold:$ncpathold $ncpathnew
	#enter nc directory
	cd $ncpathnew
	#disable maintenancemode
	sudo -u www-data php occ maintenance:mode --off
	#run the fingerprint maintenance occ
